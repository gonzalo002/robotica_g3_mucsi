...
```bash
roslaunch proyecto_final figure_maker.launch cam_tracker:=true cam_view:=true rob_sim:=false
```
...
- Este launch file tiene tres argumentos de entrada cam_tracker (activar la cámara superior para localizar cubos), cam_view (activar las tres cámaras para obtener las vistas) y rob_sim (Lanzar el robot en modo simulación), las cámaras están por defecto con valor true, mientras que el robot esta a false; solo pueden tener como parámetros válidos: true o false. Según cómo se ajuste se lanzarán las cámaras o no, y en el caso del robot se lanzara el control del robot real o la simulación. Finalmente, en el caso de las cámaras, si se lanzan con valor false al envíar el request de alguno de los actions servers se indicara el número de imagen que se quiere analizar.
