...
def hand_data_callback(msg, control_robot):
    ...
    if is_open:
        ...
        delta_x = x * rango_x_robot / rango_x_cam
        delta_y = y * rango_y_robot / rango_y_cam

        new_x = current_pose.position.x + delta_x
        new_y = current_pose.position.y + delta_y

        if new_x > limitesx[0] or new_x < limitesx[1]:
            rospy.logwarn(f"Posición X fuera de límites: {new_x}")
        elif new_y > limitesy[0] or new_y < limitesy[1]:
            rospy.logwarn(f"Posición Y fuera de límites: {new_y}")
        else:
            new_pose = copy.deepcopy(current_pose)
            new_pose.position.x = new_x
            new_pose.position.y = new_y
            listpose = [current_pose, new_pose]

            success = control_robot.move_trajectory(listpose)
            if success:
                rospy.loginfo("El robot se ha movido basado en la posición de la mano.")
            else:
                rospy.logwarn("No se pudo mover el robot a la nueva posición.")
    else:
