...
class SecuenceCommander:
    ...
    def __init__(self, simulation:bool = False) -> None:
        ...
        self.control_por_mano_activado = False
        self.altura_z = 0.237
        self.name = "RobotMain"
        ...
        self.hand_pose:list = [0.0, 0.0, 0.0] # x, y, z
    ...
    def _moveJoint(self, jointstate:JointState) -> bool:   
        ...
        if success == True:
            self.message =  crear_mensaje("JointState alcanzada", "INFO", self.name)
        else:
            self.message =  crear_mensaje("JointState inalcanzable", "ERROR", self.name)
        return success
    ...
    def _movePose(self, pose:Pose, wait:bool = True) -> bool:    
        ...
        if success == True:
            self.message =  crear_mensaje("La pose ha sido alcanzada.", "INFO", self.name)
        else:
            self.message =  crear_mensaje("La pose NO ha sido alcanzada.", "ERROR", self.name)
        return success
    ...
    def _empty_workspace(self) -> None:
        ...
        self.message =  crear_mensaje("Limpiando la mesa de trabajo...", "INFO", self.name)
    ...
    def _test_figure(self) -> bool:
        ...
        for i, color_id in enumerate(valores_unicos):
            if color_id == 4:
                if conteo[i] > self.available_cubes[color_id]:
                    self.message =  crear_mensaje(f"No hay suficientes cubos {self.color_name[color_id]} para generar la figura", "ERROR", self.name)
                    return False
        ...
        self.message =  crear_mensaje(f"Se disponen de suficientes cubos para generar la figura", "SUCCESS", self.name)
    ...
    def create_figure(self) -> None:
        ...
        self.message =  crear_mensaje("Empezando a crear la figura...", "INFO", self.name)
        self._moveJoint(self.j_link_1)
                        ...
                        if cube.color == figure_color:
                            ...
                            if self._pick_cube(cube.pose, cube.id):
                                if self._drop_cube(make_figure=True, cube_id=cube.id, matrix_position=[x-i, y-k, j]):
                                    ...
                                    self.message =  crear_mensaje('El robot no ha podido dejar el cubo.', 'ERROR', 'RobotMain')
                                    respuesta = self.message =  crear_mensaje('Aun así, ¿quieres continuar? (s/n)', 'INPUT', 'RobotMain')
                                    if respuesta == 'n':
                                        raise KeyError('Generación de Figura detenida')
                                    else:
                                        break 
                            else:
                                self.message =  crear_mensaje('El robot no ha podido coger el cubo.', 'ERROR', 'RobotMain')
                                respuesta = self.message =  crear_mensaje('Aun así, ¿quieres continuar? (s/n)', 'INPUT', 'RobotMain')
                                if respuesta == 'n':
                                    raise KeyError('Generación de Figura detenida')
                                else:
        ...
        if self.robot.move_carthesian_trayectory([pose_previa, pose]):
            ...
            if not self.simulation:
                ...
                if not effort and gripper < 5:
                    self.message =  crear_mensaje(f"El cubo {cube_id} NO ha sido agarrado.", "ERROR", self.name)
                    return False
                else:
                    self.message =  crear_mensaje(f"El cubo {cube_id} ha sido agarrado.", "SUCCESS", self.name)    
    ...
    def _drop_cube(self, make_figure:bool = True, cube_id:int = None, matrix_position:list = [0,0,0]) -> bool:
        ...
        if self.robot.move_carthesian_trayectory([pose_previa, pose]):
            ...
            if not self.simulation:
                ...
                self.message = crear_mensaje(f"El cubo {cube_id} ha sido soltado.", "SUCCESS", self.name)  
    ...
    def _put_calibration_pin(self):
        self.message =  crear_mensaje("¿Está el puntero de calibración posicionado?", "INPUT", self.name)
    ...
    def _leave_calibration_space(self):
        ...
        self.message =  crear_mensaje("¿Está el robot en la esquina del ArUco?", "INPUT", self.name)
    ...
    def _drop_calibration_pin(self):
        self.message =  crear_mensaje("¿Se puede soltar el puntero de calibración?:", "INPUT", self.name)
    ...
    def _hand_control_active(self) -> bool:
        self.control_por_mano_activado = True
    ...
    def _control_robot_by_hand(self) -> bool:
        if self.control_por_mano_activado: 
            if self.hand_gesture['is_open']:
                ...
                if not self._movePose(target_pose, wait=False):
                    self.message = crear_mensaje("No se pudo ejecutar el movimiento", 'ERROR', self.name)
                        
            # Verificar el estado de la pinza
            elif self.hand_gesture['is_peace']:
                self.robot.move_gripper(0.0, 10.0, sleep_after=1.5)
                self.message = crear_mensaje("Pinza cerrada - Intentando agarrar objeto", 'INFO', self.name)

            elif self.hand_gesture['is_dino']:
                self.robot.move_gripper(30.0, 20.0, sleep_after=0.5)
                self.message = crear_mensaje("Pinza abierta - Soltando objeto", 'INFO', self.name)
